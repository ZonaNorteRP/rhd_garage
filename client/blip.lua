...
function gb.refresh ( data )
    ...
    for k, v in pairs(data) do
        -- Skip blips for boat, planes and helicopter garages
        local isHiddenType = false
        if v.type and type(v.type) == 'table' then
            for _, t in ipairs(v.type) do
                if t == 'boat' or t == 'planes' or t == 'helicopter' then
                    isHiddenType = true
                    break
                end
            end
        ...
        if v.blip and not isHiddenType then
