...
if isServer then
    ...
    function fw.gpvbg(src, garage, filter)
        ...
        local selectFields = (filter and filter.minimal)
            and "plate, vehicle_name, stored, garage, fuel, engine, body"
            or "plate, vehicle, vehicle_name, stored, garage, fuel, engine, body, deformation"
            ...
            if not filter.impound then
                if filter.shared then
                    local sharedSelect = (filter.minimal)
                        and "ov.plate, ov.vehicle_name, ov.stored, ov.garage, ov.fuel, ov.engine, ov.body, u.firstname, u.lastname"
                        or "ov.plate, ov.vehicle, ov.vehicle_name, ov.stored, ov.garage, ov.fuel, ov.engine, ov.body, ov.deformation, u.firstname, u.lastname"
                ...
                format = ([[
                    SELECT
                        %s 
                    FROM owned_vehicles WHERE owner = ? AND stored = 0
                ]]):format(selectFields)
                value = {Identifier}
            end
        end
        ...
        if results and #results > 0 then
            for i = 1, #results do
                ...
                local mods = data.vehicle and json.decode(data.vehicle) or nil
                local deformation = data.deformation and json.decode(data.deformation) or nil
                local state = data.stored
                local model = mods and mods.model or nil
                local plate = data.plate
                local depotprice = data.depotprice or 0
