...
if isServer then
    ...
    ---@param src string
    ---@param garage string
    ---@param filter {impound: boolean, shared: boolean, minimal: boolean}
    function fw.gpvbg(src, garage, filter)
        local Identifier = fw.gi(src)
        if not Identifier then return {} end
        local format, value
        local selectFields = (filter and filter.minimal) 
            and "vehicle, vehicle_name, state, depotprice, plate, fakeplate, fuel, engine, body, chop_fee"
            or "vehicle, vehicle_name, mods, state, depotprice, plate, fakeplate, fuel, engine, body, deformation, chop_fee"
        ...
        if filter and not Config.VehiclesInAllGarages then
            if not filter.impound then
                if filter.shared then
                    local sharedSelect = (filter.minimal)
                        and "pv.vehicle, pv.vehicle_name, pv.state, pv.depotprice, pv.plate, pv.fakeplate, pv.fuel, pv.engine, pv.body, pv.chop_fee, p.charinfo"
                        or "pv.vehicle, pv.vehicle_name, pv.mods, pv.state, pv.depotprice, pv.plate, pv.fakeplate, pv.fuel, pv.engine, pv.body, pv.deformation, pv.chop_fee, p.charinfo"
        ...
        if results and #results > 0 then
            for i = 1, #results do
                ...
                local mods = data.mods and json.decode(data.mods) or nil
                local deformation = data.deformation and json.decode(data.deformation) or nil
                ...
                local isOut = false
                if state == 0 then
                    local outData = vehFuncS.govbp(plate)
                    if outData and outData.exist then
                        isOut = true
                    end
                end
                ...
                vehicles[#vehicles + 1] = {
                    ...
                    chop_fee = data.chop_fee or 0
                }

                if filter.shared then
                    local charinfo = data.charinfo and json.decode(data.charinfo)
                    local ownername = charinfo and ("%s %s"):format(charinfo.firstname, charinfo.lastname) or "Unknown"
